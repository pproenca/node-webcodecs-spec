...
#define WEBCODECS_ENSURE_STATE(env, actual, expected, message) WEBCODECS_ENSURE(env, actual == expected, message)
...
class BaseWorker : public Napi::AsyncWorker {
 public:
  explicit BaseWorker(const Napi::Function& callback) : Napi::AsyncWorker(callback) {}
...
// Native -> JS
inline Napi::String FromStdString(Napi::Env env, const std::string& str) { return Napi::String::New(env, str); }

inline Napi::Number FromInt64(Napi::Env env, int64_t value) {
  return Napi::Number::New(env, static_cast<double>(value));
}

inline Napi::Number FromDouble(Napi::Env env, double value) { return Napi::Number::New(env, value); }

inline Napi::Boolean FromBool(Napi::Env env, bool value) { return Napi::Boolean::New(env, value); }
